...
@dag_filter_rule
def _test_dag_filter_rule(val: dict) -> list[dict]:
    return val.get("dags", [])


@dag_rule
def _test_dag_rule(val: dict) -> OrbiterDAG:
    return OrbiterDAG(dag_id=val["dag_id"], file_path=val["__file"])


@task_filter_rule
def _test_task_filter_rule(val: dict) -> list[dict]:
    return val.get("tasks", [])


@task_rule
def _test_task_rule(val: dict) -> OrbiterOperator | None:
    return OrbiterEmptyOperator(task_id=val.get("task_id"))


@task_dependency_rule
def _test_task_dependency_rule(val: OrbiterDAG) -> List[OrbiterTaskDependency]:
    return [
...
test_ruleset = TranslationRuleset(
    file_type={FileTypeYAML},
    dag_filter_ruleset={"ruleset": [_test_dag_filter_rule]},
    dag_ruleset={"ruleset": [_test_dag_rule]},
    task_filter_ruleset={"ruleset": [_test_task_filter_rule]},
    task_ruleset={"ruleset": [_test_task_rule]},
    task_dependency_ruleset={"ruleset": [_test_task_dependency_rule]},
    post_processing_ruleset=EMPTY_RULESET,
    translate_fn=translate,
)


def test_translate(project_root):
    expected = OrbiterProject().add_dags(
        OrbiterDAG(dag_id="dag_a", file_path=Path(__file__)).add_tasks(
            [OrbiterEmptyOperator(task_id="task_a").add_downstream("task_b"), OrbiterEmptyOperator(task_id="task_b")]
        )
    )

    TranslationRuleset.get_files_with_extension = _test_file_generator
